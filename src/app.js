; (function () {
    ...
    function incognitoUrl() {
        return `file://${window.location.pathname.replace(/[^/\\]*$/, '')}incognito.html`.replace(/\\/g, '/');
    }
    ...
    // Primary path: ?incognito=1 query param set by main.js BEFORE the renderer
    // initializes (checked in init()), so no normal tabs are ever created.
    // Fallback: late 'set-incognito' IPC — close any normal tabs that already exist.
    function setIncognitoMode() {
    ...
    window.mauzer.on.incognito((val) => {
        if (!val) return;
        if (state.isIncognitoWindow) return; // already applied via query param
        setIncognitoMode();
        state.tabs.filter(t => !t.incognito).forEach(t => closeTab(t.id));
        if (!state.tabs.some(t => t.incognito)) createTab('', { incognito: true, _force: true });
    });
    ...
    async function renderSidebarPanelInner(panel) {
        ...
        if (panel === 'bookmarks') {
            ...
            c.innerHTML = `<div class="sidebar-panel-title" style="display:flex;align-items:center;justify-content:space-between;">${escapeHtml(titleLbl)}<button class="sidebar-clear-all-btn" id="history-clear-all" title="${escapeHtml(clearAllLbl)}">${escapeHtml(clearAllLbl)}</button></div>`;

            if (!h.length) { c.innerHTML += `<div class="sidebar-empty">${escapeHtml(emptyLbl)}</div>`; return; }
            ...
            sortedKeys.forEach(label => {
                ...
                header.querySelector('.sidebar-clear-day-btn').addEventListener('click', async () => {
                    ...
                    if (ids.length) {
                        if (window.mauzer.history.removeMany) {
                            await window.mauzer.history.removeMany(ids);
                        } else {
                            for (const id of ids) await window.mauzer.history.remove(id);
                        }
                    }
    ...
    function renderCommands(query) {
        ...
        filtered.slice(0, 12).forEach((cmd, i) => {
            ...
            el.innerHTML = `<div class="command-item-icon">${cmd.icon || '⚡'}</div><div class="command-item-text"><div class="command-item-title">${escapeHtml(cmd.title)}</div>${cmd.desc ? `<div class="command-item-desc">${escapeHtml(cmd.desc)}</div>` : ''}</div>${cmd.kbd ? `<span class="command-item-kbd">${escapeHtml(cmd.kbd)}</span>` : ''}`;
    ...
    function bindEvents() {
        ...
        function renderUrlSuggestions(list) {
            ...
            dom.urlSuggestions.innerHTML = list.map(text => `
                <li class="suggestion-item" data-val="${escapeHtml(text)}">
    ...
    // ============================================================
    async function init() {
        // Incognito flag must be applied before any tab/session work.
        // main.js passes it as a query param when creating the window.
        if (new URLSearchParams(window.location.search).get('incognito') === '1') {
            setIncognitoMode();
        }
